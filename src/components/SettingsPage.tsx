        ...
        <div className="flex-1 w-full max-w-5xl mx-auto p-4 md:p-8 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-[#0f172a] border border-slate-800/60 rounded-3xl p-6 shadow-xl shadow-black/20 relative overflow-hidden group">
                        ...
                        <div className="flex flex-col items-center text-center space-y-4 py-4 relative z-10">
                            <div className="relative">
                                ...
                                <div className="absolute bottom-0 right-0 w-7 h-7 bg-emerald-500 border-4 border-[#020617] rounded-full" />
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-white capitalize">{displayName}</h3>
                                <p className="text-sm text-slate-500 font-medium mt-0.5">subham019650@gmail.com</p>
                            </div>
                            <div className="flex gap-2 pt-2">
                                <span className="px-3 py-1 bg-blue-500/10 text-blue-400 text-[10px] font-black uppercase tracking-widest rounded-full border border-blue-500/20">Pro Member</span>
                                <span className="px-3 py-1 bg-slate-800 text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-full border border-slate-700">Admin</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    {/* Profile Section */}
                    <section className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-[2.5rem] p-8 space-y-8 relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 group-hover:text-slate-400 transition-colors" />
                                    <Input
                                        value="subham019650@gmail.com"
                                        readOnly
                                        className="bg-[#0b1120]/50 border-slate-800 text-slate-400 h-12 pl-11 rounded-2xl focus:ring-0 cursor-default"
                                    />
                                </div>
                            </div>
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    className="bg-[#0b1120]/80 border-slate-800 hover:border-blue-500/30 text-white h-12 px-5 rounded-2xl focus:ring-blue-500/20 focus:border-blue-500/50 transition-all font-medium"
                                />
                            </div>
                        </div>
                    ...
                    {/* Appearance Section */}
                    <section className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-[2.5rem] p-8 space-y-8">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-2xl bg-purple-600/10 flex items-center justify-center">
                                <Palette className="w-5 h-5 text-purple-400" />
                            </div>
                            <div>
                                <h2 className="text-lg font-bold text-white tracking-tight">Visual Identity</h2>
                        ...
                        <div className="space-y-8">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Interface Mode</label>
                                <div className="grid grid-cols-2 gap-4 p-1.5 bg-[#0b1120]/50 rounded-3xl border border-slate-800/60 max-w-sm">
                                    <button
                                        onClick={() => setTheme("light")}
                                        className={`flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all duration-300 ${theme === "light" ? "bg-white text-slate-950 shadow-xl scale-100" : "text-slate-500 hover:text-slate-300 scale-[0.98]"} `}
                                    >
                                        <Sun className={`w-4 h-4 ${theme === "light" ? "fill-orange-400 text-orange-400" : ""}`} /> Light
                                    </button>
                                    <button
                                        onClick={() => setTheme("dark")}
                                        className={`flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all duration-300 ${theme === "dark" ? "bg-blue-600 text-white shadow-xl scale-100 shadow-blue-500/20" : "text-slate-500 hover:text-slate-300 scale-[0.98]"} `}
                                    >
                            ...
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Accent Palettes</label>
                                    ...
                                    {colorThemes.map((t) => (
                                        <button
                                            ...
                                            className={`flex items-center gap-3 p-4 rounded-3xl border transition-all duration-300 hover:scale-105 active:scale-95 ${colorTheme === t.name
                                                ? "bg-[#0b1120] border-blue-500 shadow-xl shadow-blue-500/10 ring-1 ring-blue-500/20"
                                                : "bg-[#0b1120]/50 border-slate-800/60 text-slate-400 hover:border-slate-700"
                                                } `}
                                        >
                                            <div className={`w-3.5 h-3.5 rounded-full ${t.bg} shadow-lg shadow-black/20`} />
                                            <span className={`text-xs font-bold ${colorTheme === t.name ? "text-white" : "text-slate-400"}`}>{t.name}</span>
                                            {colorTheme === t.name && (
                    ...
                    {/* Security & Danger Zone Combined */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
                        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-[2.5rem] p-8 flex flex-col justify-between space-y-6">
                            <div className="space-y-1">
                            ...
                            <button
                                onClick={onSignOut}
                                className="w-full flex items-center justify-center gap-3 h-12 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-2xl text-white transition-all font-bold group"
                            >
                                <LogOut className="w-4 h-4 text-slate-400 group-hover:text-white group-hover:-translate-x-1 transition-transform" />
                                Sign Out
                            </button>
                        </div>

                        <div className="bg-red-950/5 border border-red-900/20 rounded-[2.5rem] p-8 flex flex-col justify-between space-y-6">
                            <div className="space-y-1">
                                <h3 className="text-lg font-bold text-red-500">Danger Zone</h3>
                                <p className="text-sm text-red-950/60 font-medium">Permanently delete your entire data vault</p>
                            </div>
                            <Button
                                variant="destructive"
                                className="w-full bg-red-600/10 hover:bg-red-600 border border-red-900/30 text-red-500 hover:text-white font-bold h-12 rounded-2xl shadow-lg shadow-red-500/5 transition-all"
                            >
